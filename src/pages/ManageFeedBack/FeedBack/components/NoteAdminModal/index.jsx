...
const ModalNoteAdmin = (props) => {
  ...
  useEffect(() => {
    if (item !== null) {
      ...
      const getItemStatus = OPTIONS_SELECT_STATUS_HANDLE_FEEDBACK.filter(a => a.value === item.status_handle);
  ...
  const handleOk = () => {
    ...
    item.status_handle = updateStatus.value
    if (props.handleOk) {
      props.handleOk(item)
    }
  };
    ...
    <>
      <Modal title="Cập nhật ghi chú"
        ...
        <div className='card-status-handle-review'>
          {OPTIONS_SELECT_STATUS_HANDLE_FEEDBACK.map((item) => (
