...
import EditTimeOrderSchedule from "../../../container/System/DetailsOrder/EditTimeOrderSchedule";
...
const DetailOrder = () => {
  ...
  const [arrEventPromotion, setArrEventPromotion] = useState([]);
  ...
  useEffect(() => {
    ...
    dataGroup?.service?.optional_service?.map((item) => {
      item?.extend_optional?.map((i) => {
        tempArrExtend.push(i);
      });
    });
    const tempArrEventPromotion = [];
    // let tempTotalDiscount = 0;
    dataGroup?.event_promotion?.map((item) => {
      // console.log("item ", item);
      tempArrEventPromotion.push({
        discount: item.discount,
        title: item?._id?.title["vi"],
      });
    });
    let tempPrice = dataGroup.initial_fee;
    dataGroup.service_fee.map((item) => {
      tempPrice += item?.fee;
    });
    // setTotalDiscount(tempTotalDiscount);
    setPrice(tempPrice);
    setArrExtend(tempArrExtend);
    setArrEventPromotion(tempArrEventPromotion);
  }, [dataGroup]);
  console.log("====>>>>> ", arrEventPromotion);
  const timeWork = (data) => {
        ...
        <Tabs defaultActiveKey="1" size="large">
          ...
          >
            <div className="div-container">
              ...
              <div className="detail-order_info-order">
                <p className="detail-order_info-order_lable">
                  Thông tin đơn hàng
                </p>
                <div className="div-flex-row">
                  <div className="detail-order_info-order_detail-address-time">
                    <div className="div-flex-row">
                      <p>Loại dịch vụ</p>
                      <p>Theo giờ</p>
                    </div>
                    ...
                    <div className="div-flex-row">
                      <p>Địa chỉ</p>
                      <p>{dataGroup.address}</p>
                    </div>
                  </div>
                  <div className="detail-order_info-order_detail-service">
                    <div>
                      <p>Dịch vụ đã chọn:</p>
                      {arrExtend.map((item) => {
                        return (
                    ...
                    <div>
                      ...
                      <div className="div-flex-row">
                        <p>Mã giảm giá</p>
                        <p>{dataGroup?.code_promotion?.code}</p>
                        <p>{dataGroup?.code_promotion?.discount}</p>
                      </div>
                      <div>
                        <p>Chương trình khuyến mãi</p>
                        {arrEventPromotion.map((item, index) => {
                          return (
                    ...
                    <div className="detail-order_price">
                      <p>Tạm tính</p>
                      <div className="div-flex-row">
                        <p>Phí hệ thống</p>
                        <p>{systemFee}</p>
