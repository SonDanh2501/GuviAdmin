...
import { SELECT_TIME, TYPE_VIEW_OPTIONAL_SERVICE, DATE_OF_WEEK, MONTH_SCHEDULE } from "../../../../@core/constant/service.constant.js"
import "./index.scss";
...
const DateWorkComponent = (props) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 6; i++) {
      const dateStartMonth = new Date(dateNow.getFullYear(), dateNow.getMonth() + i, 1)
      const payload = {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < tempSelect.length; i++) {
      for (let y = 0; y < tempSelect[i].time.length; y++) {
        ...
        // const temp2 = new Date(temp).getTime() - timeZoneLocal
        const temp2 = new Date(temp).getTime()

        tempSelect[i].time[y].value = new Date(temp2).toISOString().slice(11, 16);
      }
    }
  ...
  useEffect(() => {
    if (time !== null && date !== null) {
      ...
      if (service.type === "loop" || service.type === "single") {
        let tempDate = new Date(new Date(date).getTime() + timeZoneLocal).toISOString();
        ...
        if (props.changeTimeSchedule) props.changeTimeSchedule(tempTimeSchedule);
      }
  ...
  useEffect(() => {
    if (time !== null && monthSchedule !== null && dateOfWeek !== null) {
      ...
      if (service.type === "schedule") {
        ...
        const dateEndSchedule = new Date(dateNow.getFullYear(), temp, dateNow.getDate(), hourTime[0], hourTime[1])
        const startDate = new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate(), hourTime[0], hourTime[1])
        let currentDate = startDate.getTime();

        while (currentDate < dateEndSchedule.getTime()) {
          currentDate += 24 * 60 * 60 * 1000;
          const tempDateTime = new Date(currentDate);
          const findDate = dateOfWeek.findIndex(x => x === tempDateTime.getDay());
          if (findDate > -1) {
  ...
  useEffect(() => {
    if (timeSchedule.length > 0 && service !== null && service.type === "schedule") {
      const tempTime = time.split(":")
      let tempSchedule = listSchedule;
      for (let i = 0; i < tempSchedule.length; i++) {
        for (let y = 0; y < tempSchedule[i].date.length; y++) {
          tempSchedule[i].date[y].timeISOSTring = new Date(tempSchedule[i].date[y].year, tempSchedule[i].date[y].month, tempSchedule[i].date[y].date, tempTime[0], tempTime[1]).toISOString()
        }
      }
                  ...
                  <>
                    <div className="date-schedule div-flex-column">
                      <div>
                        ...
                        <div className="content-date-of-week">
                          {DATE_OF_WEEK.map((item) => (
                            <>
                              <div className={`${(dateOfWeek.findIndex(x => x === item.value) > -1) ? "item-selected" : ""} item-date-of-week`} onClick={() => { onClickDateOfWeek(item.value) }}>
                                <p>{item.label}</p>
                              </div>
                            </>
                          ))}
                      ...
                      <div className="div-flex-column">
                        {listTimeQuickSelect.map((groupTime) => (
                          <div className="div-flex-row flex-justify-content-flex-start flex-wrap">
                            {groupTime.time.map((item) => (
                              <div className={`${(item.value === time) ? "item-selected" : ""} div-select-time`} onClick={() => { setTime(item.value) }}>
                                {item.label}
                              </div>
                            ))}
                          </div>
                      ...
                      <div>
                        ...
                        <div className="content-month-schedule">
                          {MONTH_SCHEDULE.map((item) => (
                            <>
                              <div className={`${(monthSchedule === item.value) ? "item-selected" : ""} item`} onClick={() => { onClickMonthSchedule(item.value) }}>
                                <p>{item.label}</p>
                              </div>
                            </>
                          ))}
                      ...
                      <Button onClick={() => { setIsOpenDetailSchedule(!isOpenDetailSchedule) }}>
                        Xem lịch làm việc
                      </Button>
                    </div>
                  </>
                        ...
                        <>
                          <div className="date-of-week">
                            ...
                            <div className="div-active-loop-week">
                              ...
                              <Switch checked={service.is_auto_order} onChange={() => { onChangeLoopDate(!service.is_auto_order) }} />
                            </div>
        ...
        <div className="date-detail-schedule">
...

          {
            listSchedule.map((monthItem) => (
              <>
                <p className="div-title-month">Tháng {monthItem.month + 1}, {monthItem.year}</p>
                    ...
                    <>
                      <div className="div-flex-column column-week">
                        ...
                        {monthItem.date.map((dateItem, index) => (
                          <>
                            {monthItem.date.findIndex(x => x.dateOfWeek === item.value) === index && monthItem.firstLastWeek < index && index < 7 ? (<div className="empty-date"></div>) : (<></>)}
                            {dateItem.dateOfWeek === item.value ? (
                              <>
                                <p className={`div-date-schedule ${(timeSchedule.indexOf(dateItem.timeISOSTring) > -1) ? "div-date-selected" : ""}`} onClick={() => { changeDateSchedule(dateItem) }}>{dateItem.date}</p>
                              </>
                            ) : (<></>)}
                          </>
                        ))}
