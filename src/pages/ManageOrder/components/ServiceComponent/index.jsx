...
const ServiceComponent = (props) => {
        ...
        for (let i = 0; i < tempService.optional_service.length; i++) {
            if (typeOptionalService === tempService.optional_service[i].type) {
                for (let y = 0; y < tempService.optional_service[i].extend_optional.length; y++) {
                    if (tempService.optional_service[i].extend_optional[y]._id === item._id) {
                        tempService.optional_service[i].extend_optional[y].selected = !tempService.optional_service[i].extend_optional[y].selected
                    }
                }
            }
        }
        ...
        for (let i = 0; i < tempService.optional_service.length; i++) {
            if (typeOptionalService === tempService.optional_service[i].type) {
                for (let y = 0; y < tempService.optional_service[i].extend_optional.length; y++) {
                    if (tempService.optional_service[i].extend_optional[y]._id === item._id) {
                        // tempService.optional_service[i].extend_optional[y].selected = !tempService.optional_service[i].extend_optional[y].selected
                        tempService.optional_service[i].extend_optional[y].count = (counter === "+") ? ++tempService.optional_service[i].extend_optional[y].count : --tempService.optional_service[i].extend_optional[y].count
                    }
                }
            }
        }
    ...
    const SelectHorizontalNoThumbnail = ({ optionalService }) => {
        return (
            <>
                {optionalService.extend_optional.map((extend) => (
                    <>
                        <div className={`${extend.selected === true ? "item-selected" : ""} item`} onClick={() => { clickSelectExtend(optionalService.type, extend) }}>
                            <p>
                                {extend.title.vi}
                            </p>
                        </div>
    ...
    const MultiSelectHorizomtalThumbnail = ({ optionalService }) => {
        return (
            <>
                {optionalService.extend_optional.map((extend) => (
                    <>
                        <div className={`${extend.selected ? "item-selected" : ""} item`} onClick={() => { clickMultiSelectExtend(optionalService.type, extend) }}>
                            {/* <img src={ extend.selected ? extend.thumbnail_active : extend.thumbnail} /> */}
                            <p>
                                {extend.title.vi}
                            </p>
                ...
                {optionalService.extend_optional.map((extend) => (
                    <>
                        <div className="div-flex-row">
                            <p>{extend.title.vi}</p>
                            <Switch checked={extend.selected} onChange={() => { toggleSwitchExtend(optionalService.type, extend) }} />
                        </div>
    ...
    const MultiSelectCountSofa = ({ optionalService }) => {
        return (
            <>
                {optionalService.extend_optional.map((extend) => (
                    <>
                        <div className={`${extend.selected ? "item-selected" : ""} item`}  >
                            <div className="text-checkbox">
                                ...
                                <div className="div-checkbox">
                                    <Checkbox checked={extend.selected} onChange={() => { onChangeCheckBoxExtend(optionalService.type, extend) }}></Checkbox>
                                </div>
                            </div>
                            ...
                            {extend.selected === true ? (
                                <>
                                    <div className="counter-optional">
                                        <Button onClick={() => { counterNumber(optionalService.type, extend, "-") }}>-</Button>
                                        <InputNumber value={extend.count} controls={false} readOnly={true} />
                                        <Button onClick={() => { counterNumber(optionalService.type, extend, "+") }}>+</Button>
                                    </div>
                                </>
                            ) : (<>
                ...
                <>
                    <div className="div-flex-row">
                        <div className="optional-service div-flex-column">
                            {service.optional_service.map((item) => (
                                <>
