...
import { getAddressCustomerApi, createOrderApi, checkEventCodePromotionOrderApi, checkCodePromotionOrderApi, getServiceFeeOrderApi } from "../../../api/order"
import { googlePlaceAutocomplete, getPlaceDetailApi } from "../../../api/location"
import { getCalculateFeeApi, getPromotionByCustomerApi } from "../../../api/service"
...
const CreateOrder = () => {
  ...
  useEffect(() => {
    ...
    if(serviceData !== null && addressEncode !== null && listExtend.length > 0 && customer !== null && paymentMethod !== null) {
      let tempPayload = {};
      ...
      if (selectCodePromotion !== null) tempPayload["code_promotion"] = selectCodePromotion.code.toString() || "";
  ...
  }, [addressEncode, listExtend, customer, dateWorkSchedule, paymentMethod, selectCodePromotion, collaborator, serviceData])



  useEffect(() => {
    if (serviceData !== null && customer !== null && dateWorkSchedule.length > 0 && addressEncode !== null) {
  ...
  const getDataOptionalService = async () => {
    ...
    for (let i = 0; i < res.data.length; i++) {
      ...
      resExtend.data.forEach(function (element) {
        element["selected"] = (element.status_default && element.status_default === true) ? true : false;
        element["count"] = 1;
      });
  ...
  const getDataCodePromotionAvaiable = async () => {
    const res = await getPromotionByCustomerApi(customer, 0, 50, serviceData._id);
  ...
  const changeService = (tempService) => {
    ...
    for (let i = 0; i < tempService.optional_service.length; i++) {
      for (let y = 0; y < tempService.optional_service[i].extend_optional.length; y++) {
        if (tempService.optional_service[i].extend_optional[y].selected === true) {
          temp.push({
      ...
      <div className="div-container-content">
        ...
        <div className="div-flex-row">
          <div className="content-create-order">
            ...
            <div className="div-flex-column">
              <p>Địa chỉ</p>
              <Input value={searchAddress} onChange={(event) => { setAddressEncode(null); setSearchAddress(event.target.value); handleSearchAddress(event.target.value) }}>
              </Input>
                    ...
                    <>
                      <div className="div-list-address-search">
                        ...
                        {listAddress.map((item) => (
                          <>
                            <div class="div-item-address" onClick={() => { selectAddress(item) }}>
                              <i class="uil uil-map-marker"></i>
                              <div class="div-name-address">
                                <p class="title-address">{item.address.split(",")[0]}</p>
                                <p class="title-details-address">{item.address}</p>
                              </div>
                            </div>
                          </>
                        ))}
                    ...
                    <>
                      <div className="div-list-address-default">
                        ...
                        {listAddressDefault.map((item) => (
                          <>
                            <div class="div-item-address" onClick={() => { selectAddressDefault(item) }}>
                              <i class="uil uil-map-marker"></i>
                              <div class="div-name-address">
                                <p class="title-address">{item.address.split(",")[0]}</p>
                                <p class="title-details-address">{item.address}</p>
            ...
            <div className="div-flex-column">
              ...
              <ServiceComponent serviceData={serviceData} changeService={(value) => { changeService(value) }}></ServiceComponent>

              <DateWorkComponent serviceData={serviceData} changeTimeSchedule={setDateWorkSchedule} />
...
{serviceData && serviceData.type !== "schedule" ? (
  <>
            ...
            <div>
              <TextArea allowClear rows={4} value={note} onChange={(event) => { setNote(event.target.value) }} />
            </div>

            <div className="list-code-promotion-available">

              {listShowCodePromotion.map((item) => (
                <>
                  <div className={`${(selectCodePromotion !== null && item._id === selectCodePromotion._id) ? "item-selected" : ""} item`} onClick={() => { setSelectCodePromotion(item) }}>
            ...
            <div className="div-flex-column fee-order">
              ...
              {serviceFee.map((item) => (
                <>
                  <div className="div-flex-row service-fee">
                    <p>{item.title.vi}:</p>
        ...
        <div className="div-flex-row">
          <Button onClick={() => { createOrder() }} >Đăng việc</Button>
        </div>
